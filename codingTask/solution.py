...
while count < countLimit:
    ...
    # 1. File folder
    folder = coolType
    # Create a folder if not exists in current working path
    if os.path.exists(folder) is False: os.makedirs(folder)
    ...
    if row > 0 and plantCode == res[row-1][0]:
        srcfile = os.path.join(str(res[row-1][2]), fileName)
